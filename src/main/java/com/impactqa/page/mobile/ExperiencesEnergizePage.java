...
public class ExperiencesEnergizePage extends BasePage{

    private static final String PageObjectRepoFileName = "ExperiencesScreenPage.xml";

    public ExperiencesEnergizePage(AppiumDriver driver, PageObjectRepoHelper.PLATFORM platform) {
        super(driver, PageObjectRepoFileName, platform);
    }

    @Step("Select the Energize form list of Experiences")
    public void tapOnEnergize()
    {
        appiumUtils.isElementDisplayed("Energize", 20);
        appiumUtils.click("Energize");
    }

    @Step("Play any random track form Energize tracks list")
    public void selectRandomTracksFromEnergizeAndPlay() throws InterruptedException {
        List<String> allTracks = appiumUtils.getTextOfListElements("allTracks");
        for (int i = 0; i < allTracks.size(); i++) {
            Allure.step("Energize tracks list : " + allTracks);
